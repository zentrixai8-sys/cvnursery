      ...
      <div className="space-y-6 pb-20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
           <div className="bg-white rounded-3xl p-6 shadow-sm border border-white/60">
              ...
              <h3 className="text-3xl font-black tracking-tighter text-rose-600">₹{totalPending.toLocaleString()}</h3>
           </div>
           <div className="bg-white rounded-3xl p-6 shadow-sm border border-white/60">
              <div className="w-10 h-10 bg-amber-50 text-amber-600 rounded-xl flex items-center justify-center mb-4">
                 <Clock className="w-5 h-5" />
              </div>
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Pending Accounts</p>
              <h3 className="text-3xl font-black tracking-tighter text-slate-900">{orders.filter(o => o.pending_amount > 0).length} Cases</h3>
           </div>
           <div className="md:col-span-2 bg-[#1A1A1A] rounded-[2rem] p-6 shadow-2xl text-white flex items-center justify-between relative overflow-hidden group">
              <div className="relative z-10">
                <p className="text-[10px] font-black text-white/40 uppercase tracking-widest mb-1">Critical Follow-ups</p>
                <h3 className="text-4xl font-black tracking-tighter text-rose-400">{criticalFollowups} High Priority</h3>
        ...
        {/* Toolbar */}
        <div className="flex flex-col md:flex-row gap-4 bg-white p-2 rounded-2xl border border-white shadow-sm">
           ...
           <div className="flex gap-2">
              <button className="px-6 bg-white border border-slate-100 rounded-xl flex items-center gap-2 text-[10px] font-black uppercase tracking-widest hover:bg-slate-50">
                 <Filter className="w-3 h-3" /> Filter
              </button>
              <button className="px-6 bg-indigo-600 text-white rounded-xl flex items-center gap-2 text-[10px] font-black uppercase tracking-widest shadow-lg shadow-indigo-100 hover:bg-indigo-700 transition-colors">
                  ...
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                     ...
                     <div className="grid grid-cols-3 gap-8 flex-1">
                        ...
                        <div>
                           ...
                           <p className="text-sm font-black text-emerald-600">₹{order.received_amount?.toLocaleString()}</p>
                        </div>
                        <div>
                           <p className="text-[8px] font-black text-rose-400 uppercase tracking-widest mb-1">Pending Dues</p>
                           <p className="text-base font-black text-rose-600">₹{order.pending_amount?.toLocaleString()}</p>
                        </div>
                     </div>
